...
contract DeftLiquidityMigration {
    ...
    */
    ...
    function getPrice() public view returns (uint256) {
        IUniswapV2Factory iFactory = IUniswapV2Factory(
            UNISWAP_V2_FACTORY_ADDRESS
        );
        address pairAddr = iFactory.getPair(
            DEFT_TOKEN_ADDRESS,
            WETH_TOKEN_ADDRESS
        );

        uint256 wethBalance = IWeth(WETH_TOKEN_ADDRESS).balanceOf(pairAddr);
        uint256 deftBalance = IWeth(DEFT_TOKEN_ADDRESS).balanceOf(pairAddr);
        uint256 initialPrice = (wethBalance * DENORM) / deftBalance;
        return initialPrice;
    }

    function migrateDeft(uint256 amountToMint) public isOwner {
        ...
        iDeft.approve(UNISWAP_V2_ROUTER_ADDRESS, type(uint256).max);
        ...
        uint256 initialWethBalance = IWeth(WETH_TOKEN_ADDRESS).balanceOf(
            pairAddr
        );
        uint256 initialDeftBalance = IWeth(DEFT_TOKEN_ADDRESS).balanceOf(
            pairAddr
        );
        ...
        uint256 finalWethBalance = IWeth(WETH_TOKEN_ADDRESS).balanceOf(
            pairAddr
        );
        uint256 finalDeftBalance = IWeth(DEFT_TOKEN_ADDRESS).balanceOf(
            pairAddr
        );

        uint256 shouldBeDeftBalance = (initialDeftBalance * finalWethBalance) /
            initialWethBalance;
        uint256 amountToBurn = finalDeftBalance - shouldBeDeftBalance;
