...
contract CerbyWrappingService is AccessControlEnumerable {
    ...
    event LockedTokenAndMintedCerbyWrappedToken(
        address fromToken,
        address toToken,
        uint256 amount
    );
    event BurnedCerbyWrappedTokenAndUnlockedToken(
        address fromToken,
        address toToken,
        uint256 amount
    );
    ...
    function bulkUpdateIsDirectionAllowed(
        DirectionAllowedObject[] calldata directionAllowed
    ) public onlyRole(ROLE_ADMIN) {
        for (uint256 i = 0; i < directionAllowed.length; i++) {
            updateIsDirectionAllowed(
    ...
    function lockTokenAndMintCerbyWrappedToken(
        address fromToken,
        address toToken,
        uint256 amount
    ) public {
        ...
        ICerbyBotDetection iCerbyBotDetection = ICerbyBotDetection(
            ICerbyToken(toToken).getUtilsContractAtPos(
        ...
        uint256 balanceBefore = iCerbyFrom.balanceOf(address(this));
        iCerbyFrom.safeTransferFrom(msg.sender, address(this), amount);
        uint256 balanceAfter = iCerbyFrom.balanceOf(address(this));
        require(
    ...
    function burnCerbyWrappedTokenAndUnlockToken(
        address fromToken,
        address toToken,
        uint256 amount
    ) public {
        ...
        uint256 balanceBefore = iCerbyTo.balanceOf(msg.sender);
        iCerbyTo.safeTransfer(msg.sender, amount);
        uint256 balanceAfter = iCerbyTo.balanceOf(msg.sender);
        require(
            balanceAfter + amount >= balanceBefore,
            "CWS: Unlock balance mismatch"
        );

        emit BurnedCerbyWrappedTokenAndUnlockedToken(
            fromToken,
            toToken,
            amount
        );
